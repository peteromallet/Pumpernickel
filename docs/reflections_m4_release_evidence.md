...
| 36 | Reflection foundation migration (0066 up) | found | migrations | `migrations/0066_reflection_foundation.sql` (408 lines) | T18 (migration validation), T26 (staging) |
| 37 | Reflection foundation migration (0066 down) | found | migrations | `migrations/0066_reflection_foundation.down.sql` | T18 (migration validation) |
| 38 | Reflection searchable content (0067 up) | found | migrations | `migrations/0067_reflections_searchable_content.sql` (504 lines) | T18 (migration validation), T22 (retrieval) |
| 39 | Reflection searchable content (0067 down) | found | migrations | `migrations/0067_reflections_searchable_content.down.sql` | T18 (migration validation) |
...
| 64 | Searchable migration tests | found | tests | `tests/test_migration_0067_reflections_searchable.py` | T18 |
...
| **CHECK constraint** | Migration 0066 | Migration 0046 |
...
1. **Storage**: Database CHECK constraints enforce domain-specific values at the storage layer
   - `mediator.reflection_sessions.failure_class` → only 4 reflection classes (migration 0066)
...
and `tests/test_migration_0067_reflections_searchable.py`).
...
- Static validator checks passed for all M4-specific migration assertions:
  - 0066 `failure_class` CHECK matches the approved reflection taxonomy.
  - 0066 still defines encrypted fields and retry/current indexes used by cleanup and recovery.
  - 0067 searchable-content reflection arm remains plaintext-only.
...
- The harness re-ran both migration modules and passed every static migration test:
  ...
  - `tests/test_migration_0067_reflections_searchable.py`: all 14 static tests passed.
- Down/rollback behavior is covered by the existing harness, but the live portions were skipped here because no safe scratch DSN was available:
  - 0066 live rollback tests were discovered and skipped, including `test_down_migration_removes_all_three_tables` and `test_down_migration_removes_all_six_policies`.
  - 0067 live scratch searchable-content tests were discovered and skipped because `TEST_DATABASE_URL` was unset.
...
- **Searchable view state:** the staging database already exposed append-only
  ...
  `migration_0068_applied_during_probe=false`.
