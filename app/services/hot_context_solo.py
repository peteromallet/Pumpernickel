...
async def _build_workout_summary_block(
    ...
    *,
    ...
    Includes only local date, workout type, total duration, workout count,
    and projection state per day.  Omits raw payloads, tokens, heart-rate
    detail, device IDs, and partner-visible sharing.

    Returns None when no workout data is available (no connection or no
    normalized rows).  This runs independently of active commitments so
    the bot always sees the user's workout picture when data exists.

    IMPORTANT: This block provides informational context only.  It does
    NOT create commitments, does NOT imply that imported workouts create
