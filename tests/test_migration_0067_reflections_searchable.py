"""Static + live validation of migration 0067_reflections_searchable_content.
...
Proves that:
  ...
  - The down migration cleans up reflection rows and restores the pre-0067
...
UP_PATH = MIGRATIONS_DIR / "0067_reflections_searchable_content.sql"
DOWN_PATH = MIGRATIONS_DIR / "0067_reflections_searchable_content.down.sql"
...
def test_0067_files_exist_and_follow_0066() -> None:
    numbered = sorted(
        path.name
    ...
    assert numbered[numbered.index(up_name) - 1].startswith("0066_")
    assert UP_PATH.exists()
    assert DOWN_PATH.exists()


def test_0067_widens_both_source_type_constraints_with_reflection() -> None:
...
def test_0067_adds_reflection_arm_to_v_searchable_content() -> None:
...
def test_0067_reflection_arm_uses_plaintext_searchable_as_canonical_text() -> None:
...
def test_0067_reflection_arm_filters_only_processed_sessions() -> None:
...
def test_0067_reflection_arm_excludes_superseded_entries() -> None:
    """Only the current (latest) revision is searchable."""
    lowered = _compact(UP_SQL)
    assert "re.supersedes_entry_id is null" in lowered


def test_0067_reflection_arm_excludes_empty_plaintext() -> None:
...
def test_0067_reflection_arm_carries_required_scope_fields() -> None:
...
def test_0067_reflection_arm_media_analysis_includes_metadata() -> None:
...
def test_0067_preserves_all_existing_source_arms() -> None:
...
def test_0067_preserves_existing_visibility_filters() -> None:
...
def test_0067_down_cleans_reflection_rows_before_tightening_constraints() -> None:
...
def test_0067_down_restores_0059_source_type_contract() -> None:
...
def test_0067_down_restores_view_without_reflection_arm() -> None:
...
# ===========================================================================
# Live DB checks — apply through 0067 in a scratch schema, seed reflection
...
class TestReflectionSearchableContentDatabase:
    """Apply through 0067, seed reflection data, and verify v_searchable_content.
    ...
    @staticmethod
    async def _migrate_through_0067(conn, *, seed_before_0025: bool = True) -> None:
        """Apply all migrations through 0067 in the current search_path."""
        from tests.fixtures.postgres import _SEED_BEFORE_0025, _migration_files

        for path in _migration_files():
            if path.name.startswith("0068_"):
                break
    ...
    async def test_all_preexisting_source_types_appear_in_view(self) -> None:
        """After 0067, all seven pre-existing source types are present as
        ...
        db_name = f"veas_0067_parity_{uuid4().hex[:12]}"
        ...
        try:
            ...
            try:
                ...
                await self._migrate_through_0067(conn)
    ...
    async def test_reflection_entries_from_processed_sessions_appear(self) -> None:
        ...
        admin_dsn = os.environ["TEST_DATABASE_URL"]
        admin_conn = await _asyncpg.connect(admin_dsn, statement_cache_size=0)
        db_name = f"veas_0067_include_{uuid4().hex[:12]}"
        test_dsn = self._database_dsn(admin_dsn, db_name)
        try:
            has_vector = await admin_conn.fetchval(
                "SELECT EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'vector')"
    ...
    async def test_reflection_entries_from_non_processed_sessions_excluded(self) -> None:
        ...
        admin_dsn = os.environ["TEST_DATABASE_URL"]
        admin_conn = await _asyncpg.connect(admin_dsn, statement_cache_size=0)
        db_name = f"veas_0067_excl_{uuid4().hex[:12]}"
        test_dsn = self._database_dsn(admin_dsn, db_name)
        try:
            has_vector = await admin_conn.fetchval(
                "SELECT EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'vector')"
    ...
    async def test_superseded_reflection_entries_excluded(self) -> None:
        ...
        admin_dsn = os.environ["TEST_DATABASE_URL"]
        admin_conn = await _asyncpg.connect(admin_dsn, statement_cache_size=0)
        db_name = f"veas_0067_super_{uuid4().hex[:12]}"
        test_dsn = self._database_dsn(admin_dsn, db_name)
        try:
            has_vector = await admin_conn.fetchval(
                "SELECT EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'vector')"
    ...
    async def test_entries_with_empty_or_null_plaintext_excluded(self) -> None:
        ...
        admin_dsn = os.environ["TEST_DATABASE_URL"]
        admin_conn = await _asyncpg.connect(admin_dsn, statement_cache_size=0)
        db_name = f"veas_0067_empty_{uuid4().hex[:12]}"
        test_dsn = self._database_dsn(admin_dsn, db_name)
        try:
            has_vector = await admin_conn.fetchval(
                "SELECT EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'vector')"
    ...
    async def test_reflection_source_type_valid_in_check_constraints(self) -> None:
        ...
        admin_dsn = os.environ["TEST_DATABASE_URL"]
        admin_conn = await _asyncpg.connect(admin_dsn, statement_cache_size=0)
        db_name = f"veas_0067_checks_{uuid4().hex[:12]}"
        test_dsn = self._database_dsn(admin_dsn, db_name)
        try:
            has_vector = await admin_conn.fetchval(
                "SELECT EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'vector')"
    ...
    async def test_derivations_not_in_searchable_content(self) -> None:
        ...
        admin_dsn = os.environ["TEST_DATABASE_URL"]
        admin_conn = await _asyncpg.connect(admin_dsn, statement_cache_size=0)
        db_name = f"veas_0067_deriv_{uuid4().hex[:12]}"
        test_dsn = self._database_dsn(admin_dsn, db_name)
        try:
            has_vector = await admin_conn.fetchval(
                "SELECT EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'vector')"
