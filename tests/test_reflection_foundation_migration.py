"""Static + live validation of migration 0066_reflection_foundation.
...
UP_PATH = MIGRATIONS_DIR / "0066_reflection_foundation.sql"
DOWN_PATH = MIGRATIONS_DIR / "0066_reflection_foundation.down.sql"
...
def test_0066_files_exist():
    assert UP_PATH.exists()
    assert DOWN_PATH.exists()


def test_0066_up_transaction_wrapping():
    assert "begin;" in ud
    assert "commit;" in ud


def test_0066_down_transaction_wrapping():
    assert "begin;" in dd
    assert "commit;" in dd


def test_0066_creates_exactly_three_reflection_tables():
...
def test_0066_source_message_arrays():
    assert "source_message_ids uuid[]" in ud


def test_0066_scope_foreign_keys():
...
def test_0066_encrypted_columns():
...
def test_0066_rls_all_three_tables():
...
def test_0066_down_all_drops_use_if_exists():
...
def test_0066_down_drops_all_six_policies():
    assert dd.count("drop policy if exists") == 6


def test_0066_session_lifecycle_checks():
...
def test_0066_derivation_checks():
...
def test_0066_all_required_indexes():
    for idx in REFLECTION_INDEXES:
        assert idx in ud, f"Missing index: {idx}"


# ===========================================================================
# Live DB checks — apply 0066 in a scratch schema and verify the surface,
# then apply the down migration and verify clean rollback.
# ===========================================================================


class TestReflectionMigrationDatabase:
    """Apply 0066 to a scratch schema and verify the live schema shape.
