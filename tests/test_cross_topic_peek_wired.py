...
async def test_cross_topic_peek_dyad_flag_on() -> None:
    ...
    assert (
        len(hc.cross_topic_peek) > 0
    ), "Expected non-empty cross_topic_peek with flag=True"
    assert any(
        item["slug"] == "career" for item in hc.cross_topic_peek
...
async def test_cross_topic_peek_dyad_flag_off() -> None:
    ...
    assert (
        len(hc.cross_topic_peek) == 0
    ), "Expected empty cross_topic_peek with flag=False"
