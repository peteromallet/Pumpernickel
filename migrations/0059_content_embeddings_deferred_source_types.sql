-- 0059_content_embeddings_deferred_source_types: extend unified embeddings to
-- deferred M4 source types.
--
-- 0058 introduced the generalized ``content_embeddings`` / ``embed_jobs``
-- contract plus ``mediator.v_searchable_content``.  This follow-up widens the
-- durable ``source_type`` checks to include ``conversation_note`` and
-- ``theme`` and replaces the searchable-content view so those rows participate
-- in retrieval without changing the existing message / memory / observation /
-- distillation / artifact visibility rules.
--
-- Conversation-note searchability is inherited from the parent conversation:
-- notes have no soft-delete state of their own, so the only content gate here
-- is non-empty trimmed text plus the conversation join that supplies bot/topic
-- scope. Themes remain topic-scoped via active ``artifact_topics`` rows and
-- only active themes are searchable.
...
COMMENT ON VIEW mediator.v_searchable_content IS
    'Unified M4 retrieval read surface for messages, memories, observations, distillations, conversation notes, themes, and artifacts. Excludes deleted/suppressed messages, inactive durable rows, deleted artifacts, empty conversation notes, and dyad_shareable non-message content.';
