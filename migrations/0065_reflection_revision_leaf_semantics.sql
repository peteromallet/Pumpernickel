-- 0065_reflection_revision_leaf_semantics: make corrected reflection entries
...
-- that no successor references.  Migration 0064 accidentally selected rows
-- whose own pointer was NULL, which selected the original revision instead.

BEGIN;

ALTER VIEW mediator.v_searchable_content
    RENAME TO v_searchable_content_pre_0065;

CREATE VIEW mediator.v_searchable_content AS
SELECT previous.*
FROM mediator.v_searchable_content_pre_0065 previous
...
COMMENT ON VIEW mediator.v_searchable_content_pre_0065 IS
    'Rollback snapshot of the 0064 searchable-content view; reflection rows are replaced by mediator.v_searchable_content.';
