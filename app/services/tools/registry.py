...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "messages_before": "Reveal the immediately earlier messages in the current topic when the hot-context gist mentions a moment but not the actual wording. Use this before guessing what came right before the current anchor.",
    "messages_after": "Reveal the immediately later messages in the current topic when the hot-context gist stops too early and you need to see what happened next around an anchor.",
    "open_thread": "Open one participant thread around a known message, date, or the latest entry when the hot-context gist is too compressed to show the surrounding exchange.",
    "scroll": "Continue a prior nav window forward or backward using its cursor when the hot-context gist is insufficient and you need more local thread/topic context without running a broad search.",
    "topic_recent": "List the newest messages in the current or specified topic when the hot-context gist is stale, partial, or missing the latest exchange details.",
    "source_messages": "Open the visible supporting messages for a search hit whose source_type is observation or distillation. Memory, artifact, and unknown source types return explicit unsupported/no-link results instead of raw source text.",
    "search": "Search the message timeline semantically or exactly when the hot-context gist is too thin to answer from memory alone. Use `scope='topic'` for the current topic or `scope='thread'` for one participant thread, then follow hit cursors with nav tools for nearby context.",
...
TOOL_DISPATCH: dict[str, ToolFn] = {
    ...
    "messages_before": read_tools.messages_before,
    "messages_after": read_tools.messages_after,
    "open_thread": read_tools.open_thread,
    "scroll": read_tools.scroll,
    "topic_recent": read_tools.topic_recent,
    "source_messages": read_tools.source_messages,
    "search": read_tools.search,
