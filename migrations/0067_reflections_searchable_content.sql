-- 0067_reflections_searchable_content: add reflection entries to the unified
-- searchable-content retrieval surface.
--
-- Extends the 0058/0059 source-type contract to include 'reflection' so
-- processed, un-superseded reflection entries participate in keyword and
-- vector retrieval through the existing mediator.v_searchable_content view.
--
-- Visibility rules:
--   * Only entries from sessions with status = 'processed' are searchable.
--     Finalizing sessions may temporarily hold entries but those are not yet
--     ready for retrieval.
--   * Only the current (un-superseded / latest) revision is searchable.
--     Corrected/superseded entries are excluded so search always surfaces the
--     most recent reflection text.
--   * Canonical search text is the entry's plaintext_searchable column, which
--     is the minimal deterministic plaintext already stored by the reflection
--     processor.  Encrypted payload_encrypted and summary_encrypted are never
--     exposed in the view.
--   * sender_id is set to entry.user_id so the existing retrieval visibility
--     filter (sender_id IN participant_ids) matches the reflection owner.
--   * thread_owner_user_id = entry.user_id carries the user scope required by
--     OOB and partner-share filters.
--   * Deferred/rejected derivation decisions are not reflected in this view;
--     they remain inspectable only through explicit list/get tools.

BEGIN;

-- 1. Widen durable source-type checks -----------------------------------------
...
-- 2. Replace the unified searchable-content view ------------------------------
...
SELECT
    ...
    c.partner_user_id AS recipient_id,
    c.user_id AS thread_owner_user_id,
    ...
    NULL::text AS media_type,
    jsonb_build_object(
        'conversation_id', cn.conversation_id,
        'attributed_to_speaker', cn.attributed_to_speaker,
        'evidence_turn_id', cn.evidence_turn_id
    ) AS media_analysis,
    c.bot_id,
    c.topic_id,
    NULL::uuid AS dyad_id,
    NULL::text AS thread_owner_partner_share,
    cn.text AS canonical_text,
    to_tsvector('simple'::regconfig, cn.text) AS search_tsv,
    cn.created_at AS sort_at,
    c.topic_id AS primary_topic_id,
    CASE WHEN c.topic_id IS NULL THEN ARRAY[]::uuid[] ELSE ARRAY[c.topic_id] END AS topic_ids,
    cn.created_at AS source_created_at,
    cn.created_at AS source_updated_at
FROM mediator.conversation_notes cn
JOIN mediator.conversations c
  ON c.id = cn.conversation_id
WHERE btrim(COALESCE(cn.text, '')) <> ''
...
SELECT
    ...
    NULL::uuid AS recipient_id,
    NULL::uuid AS thread_owner_user_id,
    t.first_seen_at AS sent_at,
    'routine'::text AS charge,
    NULL::timestamptz AS edited_at,
    NULL::jsonb AS edit_history,
    btrim(concat_ws(E'\n', t.title, t.description)) AS content,
    NULL::text AS media_type,
    NULL::jsonb AS media_analysis,
    t.recorded_by_bot_id AS bot_id,
    topics.primary_topic_id AS topic_id,
    NULL::uuid AS dyad_id,
    NULL::text AS thread_owner_partner_share,
    btrim(concat_ws(E'\n', t.title, t.description)) AS canonical_text,
    to_tsvector('simple'::regconfig, btrim(concat_ws(E'\n', t.title, t.description))) AS search_tsv,
    COALESCE(t.last_reinforced_at, t.last_active_at, t.first_seen_at) AS sort_at,
    topics.primary_topic_id,
    COALESCE(topics.topic_ids, ARRAY[]::uuid[]) AS topic_ids,
    t.first_seen_at AS source_created_at,
    COALESCE(t.last_reinforced_at, t.last_active_at, t.updated_at, t.first_seen_at) AS source_updated_at
FROM mediator.themes t
LEFT JOIN LATERAL (
    SELECT
        (array_agg(at.topic_id ORDER BY at.topic_id))[1] AS primary_topic_id,
        array_agg(at.topic_id ORDER BY at.topic_id) AS topic_ids
...
WHERE re.supersedes_entry_id IS NULL
