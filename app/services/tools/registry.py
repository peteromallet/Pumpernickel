...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "update_bridge_candidate": "Update bridge candidate lifecycle status or improve summary/note without exposing raw private material. Lifecycle statuses are exactly `pending` (drafted, not yet shareable), `ready` (cleared to send), `sent` (delivered to target), `declined` (source user refused sharing), `blocked` (OOB or sensitivity prevents sending), `addressed` (no longer needs bridging), and `expired` (stale).",
    "send_bridge_candidate": "Send a `ready` bridge candidate through the guarded outbound path using only its shareable summary. This is the only tool for sending bridge candidates across threads. Only `ready` candidates are sendable; `pending`, `blocked`, and other lifecycle statuses cannot be sent.",
